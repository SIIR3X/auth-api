...
In the currently documented model, observability is centralized, but collection remains local to each server. The application server and the data server each run their own Alloy agent. Those agents collect host metrics, journal entries, and container logs on the machine where the signals are produced, then forward them over private links to a shared Grafana, Loki, and Prometheus stack. This means the overall topology is split as follows:

\begin{itemize}
  \item the application server runs the Rust API and its local Alloy agent;
  \item the data server runs PostgreSQL and Redis as stateful backend services;
  \item the data server runs its own Alloy agent locally to collect host metrics, journal entries, and Docker logs;
  \item the central observability stack runs Loki as the shared log store;
  \item the central observability stack runs Prometheus as the shared metrics store;
  \item the central observability stack runs Grafana as the shared visualization and dashboard layer.
\end{itemize}

This topology is a better fit for the repository's current deployment model than colocating the whole monitoring stack with PostgreSQL and Redis. The application server and data server each keep a very small local footprint: only their runtime services plus one Alloy process. Heavy coordination tasks such as storage, querying, and dashboards stay centralized. As a result, observability is shared without turning either runtime host into a second control plane.
...
\midrule
Alloy & Application server and data server & Collects local host metrics, journal entries, Docker logs, and selected file logs, then forwards them with stable labels. \\
Loki & Central observability stack & Stores and indexes runtime logs for exploration and dashboard queries. \\
Prometheus & Central observability stack & Receives metrics from Alloy agents and stores time-series data. \\
Grafana & Central observability stack & Exposes dashboards, Explore queries, and the PostgreSQL datasource. \\
PostgreSQL datasource & Data server / central Grafana & Allows Grafana to query structured tables such as \texttt{audit\_log} and \texttt{login\_attempts} through a dedicated read-only role. \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}
  ...
  \item Did Nginx, the Rust API container, or Alloy fail to start correctly?
...
The deployed Alloy configurations use Docker discovery and stable labels before forwarding streams to Loki and metrics to Prometheus. The data server configuration focuses on host, journal, and Docker signals for PostgreSQL and Redis. The application server configuration adds the same host and Docker coverage, but also tails Nginx log files. This means the operator can search or graph data by container name, Docker Compose service, project, stream, host label, and the manually attached \texttt{server\_role}.

Conceptually, the pipeline is:

\begin{enumerate}
  \item Docker produces stdout/stderr log streams for each container.
  \item Alloy discovers those containers on the local host and also reads journal entries, host metrics, and selected file logs.
  \item Alloy attaches stable labels such as \texttt{service}, \texttt{container}, \texttt{host}, and \texttt{server\_role}.
  \item Loki ingests and stores the resulting log streams, while Prometheus receives the forwarded metrics.
  \item Grafana queries Loki, Prometheus, and PostgreSQL and renders dashboards or ad hoc views.
\end{enumerate}
...
At the current stage of the repository, the monitoring stack is designed to centralize at least:

\begin{itemize}
  \item Rust API container logs;
  \item Nginx logs on the application server;
  \item PostgreSQL logs;
  \item Redis logs;
  \item Alloy, Grafana, Loki, and Prometheus logs;
  \item system journal entries from the application and data servers;
  \item migration-container logs when the migrations image is executed on the data server.
\end{itemize}

This is already a meaningful baseline. It covers the public runtime host, the private stateful host, the monitoring stack itself, and the SQL audit layer without forcing all operational evidence into a single storage model.
...
\midrule
\texttt{Rust API App Server} & Prometheus and Loki & Shows host-level resource usage and runtime log activity for the application server. \\
...
\texttt{Rust API Audit Overview} & PostgreSQL & Queries structured audit and login-related tables through a read-only SQL datasource. \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}
  \item Are the application and data hosts both visible in metrics?
  \item Are services producing logs at all?
  \item Are there obvious recent errors?
  \item Can an operator immediately inspect PostgreSQL, Redis, Nginx, or application output?
\end{itemize}
...
\begin{itemize}
  \item Grafana should remain private unless intentionally published behind a secured reverse proxy;
...
This is a coherent security stance. A monitoring stack has visibility into service behavior, identifiers, and potentially sensitive failure modes. The current documentation strengthens that stance further by routing metrics and logs over private connectivity rather than relying on broad public exposure. It should therefore be treated as an operationally privileged subsystem, not as a disposable convenience service.
...
Third, it integrates naturally with the documented deployment model. The data server owns its local Alloy configuration under \texttt{/srv/rust-api-data}, the application server owns its own Alloy configuration under \texttt{/srv/rust-api/observability}, and the shared Grafana, Loki, and Prometheus stack remains a separate operational concern. This split keeps runtime hosts focused on collection and execution, while the central stack focuses on storage, querying, and dashboards.
