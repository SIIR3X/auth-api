...
model built around Docker, PostgreSQL, Redis, Nginx, \texttt{pass}, and
\texttt{cryptsetup}, and describes the monitoring stack used to observe logs and
...
\subsection{Encrypted Service Roots with \texttt{cryptsetup}}
...
with LUKS volumes managed through \texttt{cryptsetup}. This is operationally coherent with the local-secret model:

\begin{itemize}
  \item \texttt{pass} protects individual secret values;
  \item encrypted service roots protect the broader server filesystem at rest.
\end{itemize}
...
\begin{enumerate}
  \item prepare the encrypted service root if LUKS is used;
...
\begin{itemize}
  ...
  \item the local secret store and encrypted service root.
\end{itemize}
...
\begin{itemize}
  \item PostgreSQL and Redis as stateful backend services;
  \item Loki as the log store;
  \item Grafana as the visualization layer;
  \item Alloy as the local agent reading Docker container logs.
\end{itemize}
...
PostgreSQL datasource & Data server / Grafana & Allows Grafana to query structured tables such as \texttt{audit\_log} and \texttt{login\_attempts} through a dedicated read-only role. \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}
  ...
  \item Redis logs;
  \item Grafana, Loki, and Alloy logs;
...
\begin{itemize}
  ...
  \item Loki should remain private and should not be exposed broadly;
  \item Alloy's debug UI should remain local or at most private-network only;
...
Third, it integrates naturally with the documented deployment model. The stack can be deployed on the data server using the same documentation-driven workflow as the rest of the infrastructure, with local configuration files, a local \texttt{pass} store, and predictable filesystem paths under \texttt{/srv/rust-api-data}.
