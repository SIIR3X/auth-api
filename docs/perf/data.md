...
| Cœurs | 8 |
| Mémoire (Go) | 23.4 |
| Noyau | 6.12.107+deb13-amd64 |
| PostgreSQL | postgres (PostgreSQL) 17.11 |
| Commit mesuré | 3ac984d |
| Épinglage CPU | postgres=0-2;cache=3;api=4-6;load=7 |
| Pool PostgreSQL de l'API | 32 |
| Argon2id | m=65536 KiB, t=3, p=4 |
| Mesure HTTP (s) | 20 |
| Chauffe HTTP (s) | 5 |
| Mesure SQL (s) | 10 |
| Réglages PostgreSQL | `effective_cache_size=12GB`, `fsync=on`, `full_page_writes=on`, `max_connections=200`, `max_wal_size=8GB`, `random_page_cost=1.1`, `server_version=17.11`, `shared_buffers=4GB`, `synchronous_commit=on`, `work_mem=16MB` |

## Volumes de données

| Table | 10k : lignes | 10k : table / index (Mo) | 100k : lignes | 100k : table / index (Mo) | 1M : lignes | 1M : table / index (Mo) |
|---|---:|---:|---:|---:|---:|---:|
...
| **Base entière (Mo)** | 109 |  | 1 307 |  | 10 544 | |
| Durée du peuplement (s) | 5 (+10 000 comptes) |  | 38 (+90 000 comptes) |  | 546 (+900 000 comptes) | |

## API HTTP

![Débit](img/http-throughput.svg)

![Latence p95](img/http-p95.svg)

### Débit maximal par scénario

| Scénario | 10k : req/s max (clients) | 10k : p95 à ce point (ms) | 100k : req/s max (clients) | 100k : p95 à ce point (ms) | 1M : req/s max (clients) | 1M : p95 à ce point (ms) |
|---|---:|---:|---:|---:|---:|---:|
...
| Trafic mixte | 356 (64) | 1 795 | 325 (64) | 1 924 | 322 (256) | 7 749 |

### GET /users/me

| Utilisateurs | Clients | req/s | p50 (ms) | p95 (ms) | p99 (ms) | max (ms) | Erreurs | CPU API (cœurs) | CPU PostgreSQL (cœurs) | CPU Redis+NATS | Commits PG/s | Hit ratio |
|---:|---:|---:|---:|---:|---:|---:|---|---:|---:|---:|---:|---:|
...
Répartition par opération au plus fort volume et à la plus forte concurrence :

| Opération | req/s | p50 (ms) | p95 (ms) | p99 (ms) | Erreurs |
|---|---:|---:|---:|---:|---|
...
![Latence p99 des requêtes](img/db-p99.svg)

![Débit des requêtes](img/db-throughput.svg)

### Requêtes de l'application

| Requête | Utilisateurs | 1 conn. : req/s | 1 conn. : p50 / p99 (ms) | 8 conn. : req/s | 8 conn. : p50 / p99 (ms) | 32 conn. : req/s | 32 conn. : p50 / p99 (ms) | CPU PG à 32 conn. | Lectures disque/s |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| Utilisateur par e-mail | 10k | 5 821 | 0.17 / 0.22 | 21 110 | 0.39 / 0.50 | 24 078 | 1.4 / 1.7 | 1.88 | 0 |
| Utilisateur par e-mail | 100k | 5 774 | 0.17 / 0.23 | 21 252 | 0.39 / 0.50 | 24 036 | 1.4 / 1.7 | 1.91 | 0 |
| Utilisateur par e-mail | 1M | 5 660 | 0.18 / 0.25 | 21 526 | 0.38 / 0.50 | 24 155 | 1.4 / 1.7 | 1.96 | 0 |
| Session par refresh token | 10k | 6 080 | 0.16 / 0.23 | 19 280 | 0.43 / 0.53 | 20 583 | 1.6 / 2.0 | 1.60 | 0 |
| Session par refresh token | 100k | 6 122 | 0.16 / 0.22 | 19 473 | 0.43 / 0.51 | 20 138 | 1.6 / 1.8 | 1.56 | 0 |
| Session par refresh token | 1M | 5 141 | 0.17 / 0.35 | 19 823 | 0.42 / 0.52 | 20 723 | 1.6 / 1.7 | 1.62 | 155 |
| Validité d'une session | 10k | 6 309 | 0.16 / 0.22 | 20 363 | 0.41 / 0.49 | 21 132 | 1.6 / 1.8 | 1.58 | 0 |
| Validité d'une session | 100k | 6 331 | 0.16 / 0.22 | 20 508 | 0.41 / 0.49 | 21 242 | 1.6 / 1.8 | 1.61 | 0 |
| Validité d'une session | 1M | 5 605 | 0.16 / 0.30 | 20 654 | 0.40 / 0.48 | 21 886 | 1.5 / 1.7 | 1.66 | 6 |
| Sessions actives d'un compte | 10k | 6 086 | 0.16 / 0.23 | 19 115 | 0.44 / 0.53 | 19 574 | 1.7 / 1.9 | 1.53 | 0 |
| Sessions actives d'un compte | 100k | 5 972 | 0.17 / 0.23 | 18 851 | 0.44 / 0.53 | 19 706 | 1.7 / 1.9 | 1.58 | 0 |
| Sessions actives d'un compte | 1M | 5 583 | 0.17 / 0.35 | 19 379 | 0.43 / 0.53 | 20 356 | 1.6 / 1.8 | 1.63 | 88 |
| Échecs récents par identifiant | 10k | 5 885 | 0.17 / 0.23 | 21 811 | 0.38 / 0.49 | 23 820 | 1.4 / 1.7 | 1.85 | 0 |
| Échecs récents par identifiant | 100k | 5 755 | 0.17 / 0.23 | 21 993 | 0.37 / 0.52 | 24 627 | 1.4 / 1.6 | 1.90 | 0 |
| Échecs récents par identifiant | 1M | 5 677 | 0.17 / 0.30 | 22 340 | 0.37 / 0.48 | 24 860 | 1.4 / 1.6 | 1.94 | 0 |
| Échecs récents par adresse | 10k | 6 102 | 0.16 / 0.23 | 20 428 | 0.40 / 0.53 | 21 504 | 1.6 / 1.9 | 1.66 | 0 |
| Échecs récents par adresse | 100k | 5 916 | 0.17 / 0.24 | 20 633 | 0.40 / 0.50 | 22 317 | 1.5 / 1.7 | 1.72 | 0 |
| Échecs récents par adresse | 1M | 6 127 | 0.16 / 0.22 | 20 772 | 0.40 / 0.49 | 21 272 | 1.6 / 1.7 | 1.65 | 0 |
| Échecs consécutifs d'un compte | 10k | 5 746 | 0.17 / 0.24 | 22 279 | 0.37 / 0.49 | 24 100 | 1.4 / 1.6 | 1.87 | 0 |
| Échecs consécutifs d'un compte | 100k | 5 763 | 0.17 / 0.24 | 22 042 | 0.37 / 0.49 | 25 883 | 1.3 / 1.6 | 1.98 | 0 |
| Échecs consécutifs d'un compte | 1M | 4 634 | 0.19 / 0.42 | 22 523 | 0.36 / 0.50 | 25 703 | 1.3 / 1.6 | 2.03 | 366 |
| Rôles et permissions | 10k | 5 623 | 0.18 / 0.24 | 22 701 | 0.36 / 0.50 | 27 683 | 1.2 / 1.7 | 2.08 | 0 |
| Rôles et permissions | 100k | 5 579 | 0.18 / 0.25 | 22 900 | 0.36 / 0.51 | 27 752 | 1.2 / 1.6 | 2.12 | 0 |
| Rôles et permissions | 1M | 5 663 | 0.18 / 0.24 | 22 916 | 0.36 / 0.48 | 25 933 | 1.3 / 1.6 | 2.03 | 0 |
| Page d'historique (50) | 10k | 4 936 | 0.20 / 0.28 | 18 180 | 0.45 / 0.62 | 19 681 | 1.7 / 2.0 | 1.92 | 0 |
| Page d'historique (50) | 100k | 4 753 | 0.21 / 0.29 | 17 977 | 0.45 / 0.65 | 21 173 | 1.6 / 2.1 | 2.12 | 0 |
| Page d'historique (50) | 1M | 2 692 | 0.32 / 1.2 | 18 268 | 0.45 / 0.64 | 22 450 | 1.5 / 2.0 | 2.26 | 89 |
| Seconds facteurs et codes | 10k | 3 181 | 0.31 / 0.39 | 10 610 | 0.76 / 0.95 | 11 439 | 2.8 / 3.4 | 1.61 | 0 |
| Seconds facteurs et codes | 100k | 3 124 | 0.31 / 0.46 | 10 426 | 0.77 / 0.93 | 11 255 | 2.8 / 3.4 | 1.58 | 0 |
| Seconds facteurs et codes | 1M | 3 097 | 0.32 / 0.47 | 10 676 | 0.75 / 0.94 | 12 315 | 2.7 / 3.4 | 1.79 | 343 |
| Écritures d'une connexion (tx) | 10k | 500 | 1.7 / 3.5 | 2 850 | 3.1 / 4.4 | 6 422 | 4.9 / 8.2 | 2.31 | 0 |
| Écritures d'une connexion (tx) | 100k | 447 | 2.0 / 3.3 | 2 807 | 3.1 / 4.4 | 6 493 | 4.9 / 8.0 | 2.35 | 0 |
| Écritures d'une connexion (tx) | 1M | 338 | 2.8 / 4.4 | 1 863 | 4.2 / 7.5 | 4 805 | 6.8 / 11.0 | 2.67 | 11 420 |

### Plans d'exécution

Médiane de 7 exécutions `EXPLAIN (ANALYZE, BUFFERS)` pour un compte au milieu de la plage.

| Requête | 10k : ms (blocs lus) | 100k : ms (blocs lus) | 1M : ms (blocs lus) | Plan au plus fort volume |
|---|---:|---:|---:|---|
...
### Plus gros index à 1M utilisateurs

| Index | Table | Taille (Mo) | Parcours |
|---|---|---:|---:|
...
### `pg_stat_statements` à 10k utilisateurs (mixed, 64 virtual users)

| Requête | Appels | Moyenne (ms) | Part du temps SQL | Blocs lus |
|---|---:|---:|---:|---:|
...
### `pg_stat_statements` à 100k utilisateurs (mixed, 64 virtual users)
...
### `pg_stat_statements` à 1M utilisateurs (mixed, 64 virtual users)
...
### Purge (lots de 5 000 lignes)

| Tâche | 10k : lignes / ms par lot | 100k : lignes / ms par lot | 1M : lignes / ms par lot |
|---|---:|---:|---:|
