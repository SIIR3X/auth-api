...
fn migrations_are_sorted_and_contiguous() {
    ...
    let names = files.iter().map(|file| file.file_name.as_str()).collect::<Vec<_>>();
...
fn critical_migrations_contain_expected_objects() {
    ...
    let audit_sql = migration_sql("0013_audit_log.sql");
    let login_attempts_sql = migration_sql("0012_login_attempts.sql");
    let recovery_sql = migration_sql("0011_recovery_codes.sql");
