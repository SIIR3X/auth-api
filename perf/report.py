...
QUERY_TITLES = {
    ...
    "session_by_token": "Session by refresh token",
    "session_validation": "Session validity",
    "active_sessions": "Active sessions of an account",
    "failures_by_identifier": "Recent failures by identifier",
    "failures_by_ip": "Recent failures by address",
    "consecutive_failures": "Consecutive failures of an account",
    "rbac": "Roles and permissions",
    "audit_page": "History page (50)",
    "two_factor_overview": "Second factors and codes",
    "sign_in_write": "Sign-in writes (tx)",
}
...
def main():
    ...
    md += ["| Item | Value |", "|---|---|"]
    for key, label in [
        ("cpu_model", "CPU"), ("cpus", "Cores"), ("memory_gb", "Memory (GB)"),
        ("kernel", "Kernel"), ("postgres", "PostgreSQL"), ("commit", "Measured commit"),
        ("cpu_groups", "CPU pinning"), ("api_db_pool", "API PostgreSQL pool"),
        ("argon2", "Argon2id"), ("duration_secs", "HTTP measurement (s)"), ("warmup_secs", "HTTP warm-up (s)"),
        ("db_duration_secs", "SQL measurement (s)"),
    ]:
        if key in env:
    ...
    if explains:
        ...
        md.append("| PostgreSQL settings | " + ", ".join(
    ...
    header = "| Table | " + " | ".join(f"{n}: rows | {n}: table / index (MB)" for n in vnames) + " |"
    ...
    for rel in relations_of_interest:
        ...
        md.append("| " + " | ".join(row) + " |")
    md.append("| **Whole database (MB)** | " + " | ".join(
        (mb(explains[v]["database_bytes"]) if v in explains else "-") + " | " for v in volumes).rstrip(" |") + " | |")
    md.append("| Seeding time (s) | " + " | ".join(
        (f"{seeds[v]['seconds']} (+{num(seeds[v]['added'])} accounts)" if v in seeds else "-") + " | " for v in volumes).rstrip(" |") + " | |")
    md.append("")
    ...
    if http:
        small_multiples(
            ...
            "Throughput by number of concurrent clients",
            "Requests per second, successful or not, one line per user volume",
            [(SCENARIO_TITLES[s], [(n, [idx.get((s, v, c), {}).get("rps") for c in levels]) for n, v in zip(vnames, volumes)]) for s in scenarios],
            [str(c) for c in levels], vnames, False, "concurrent clients",
        )
        small_multiples(
            img / "http-p95.svg",
            "p95 latency by number of concurrent clients",
            "Milliseconds, logarithmic scale, one line per user volume",
            ...
            [str(c) for c in levels], vnames, True, "concurrent clients",
        )
        md += ["## HTTP API", "", "![Throughput](img/http-throughput.svg)", "", "![p95 latency](img/http-p95.svg)", ""]
        md += ["### Maximum throughput per scenario", ""]
        md += ["| Scenario | " + " | ".join(f"{n}: max req/s (clients) | {n}: p95 at that point (ms)" for n in vnames) + " |",
               "|---|" + "---:|---:|" * len(vnames)]
        for s in scenarios:
        ...
        for s in scenarios:
            ...
            md += ["| Users | Clients | req/s | p50 (ms) | p95 (ms) | p99 (ms) | max (ms) | Errors | API CPU (cores) | PostgreSQL CPU (cores) | Redis+NATS CPU | PG commits/s | Hit ratio |",
                   "|---:|---:|---:|---:|---:|---:|---:|---|---:|---:|---:|---:|---:|"]
            for v in volumes:
                for c in levels:
                    l = idx.get((s, v, c))
                    if not l:
                        continue
                    errors = ", ".join(f"{k}x{num(n)}" for k, n in l["errors"].items()) or "0"
                    cpu = l.get("cpu", {})
                    db = l.get("db", {})
                    md.append("| " + " | ".join([
            ...
            if s == "mixed":
                md += ["", "Breakdown by operation at the highest volume and the highest concurrency:", ""]
                top = idx.get((s, volumes[-1], levels[-1]))
                if top:
                    md += ["| Operation | req/s | p50 (ms) | p95 (ms) | p99 (ms) | Errors |", "|---|---:|---:|---:|---:|---|"]
                    for op, st in top["operations"].items():
                        md.append(f"| {op} | {num(st['rps'])} | {ms(st['latency_ms']['p50'])} | {ms(st['latency_ms']['p95'])} | {ms(st['latency_ms']['p99'])} | {', '.join(f'{k}x{n}' for k, n in st['errors'].items()) or '0'} |")
            md.append("")
    ...
    if db_lines:
        ...
        dot_plot(
            img / "db-p99.svg",
            f"p99 latency of the application queries, {ref} connections",
            "Milliseconds, logarithmic scale, one dot per user volume",
            [(QUERY_TITLES[q], [didx.get((q, v, ref), {}).get("latency_ms", {}).get("p99") for v in volumes]) for q in queries],
            vnames, "ms",
        )
        small_multiples(
            img / "db-throughput.svg",
            "Query throughput by number of connections",
            "Queries per second per volume. Reads: the generator caps at 1 core from 8 connections, so these rates are lower bounds",
            [(QUERY_TITLES[q], [(n, [didx.get((q, v, c), {}).get("rps") for c in dlevels]) for n, v in zip(vnames, volumes)]) for q in queries],
            [str(c) for c in dlevels], vnames, False, "connections", cols=4,
        )
        md += ["## Database", "", f"![Query p99 latency](img/db-p99.svg)", "", "![Query throughput](img/db-throughput.svg)", ""]
        md += ["### Application queries", ""]
        md += ["| Query | Users | " + " | ".join(f"{c} conn.: req/s | {c} conn.: p50 / p99 (ms)" for c in dlevels) + " | PG CPU at " + str(dlevels[-1]) + " conn. | Disk reads/s |",
               "|---|---:|" + "---:|---:|" * len(dlevels) + "---:|---:|"]
        for q in queries:
            for v in volumes:
    ...
    # Plans
    if explains:
        md += ["### Execution plans", "", "Median of 7 `EXPLAIN (ANALYZE, BUFFERS)` runs for an account in the middle of the range.", ""]
        names = [p["query"] for p in explains[volumes[0]]["plans"]]
        md += ["| Query | " + " | ".join(f"{n}: ms (blocks read)" for n in vnames) + " | Plan at the highest volume |",
               "|---|" + "---:|" * len(vnames) + "---|"]
        for name in names:
            row = [f"`{name}`"]
            last_plan = None
        ...
        md.append("")
        top_v = volumes[-1]
        md += [f"### Largest indexes at {users_label(top_v)} users", "", "| Index | Table | Size (MB) | Scans |", "|---|---|---:|---:|"]
    ...
    if statements:
        for st in statements:
            md += [f"### `pg_stat_statements` at {users_label(st['users'])} users ({esc(st['context'])})", ""]
            total = sum(x["total_ms"] for x in st["statements"]) or 1
            md += ["| Query | Calls | Mean (ms) | Share of SQL time | Blocks read |", "|---|---:|---:|---:|---:|"]
    ...
    if cleanups:
        md += ["### Purge (batches of 5 000 rows)", "", "| Job | " + " | ".join(f"{users_label(c['users'])}: rows / ms per batch" for c in cleanups) + " |",
               "|---|" + "---:|" * len(cleanups) + ""]
        jobs = []
        for c in cleanups:
            for b in c["batches"]:
